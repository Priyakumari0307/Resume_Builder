...
export const updateResume = async (req, res) => {
    try {
        ...
        console.log("Update Resume Request - User:", userId, "ResumeID:", resumeId);
            ...
            const response = await imageKit.files.upload({
                ...
                transformation: {
                    pre: 'w-300,h-300,fo-face,z-0.75' +
                        (removeBackground === 'true' || removeBackground === true ? ',e-bgremove' : '')
                }
            });
        ...
        // Clean up data for Mongoose update
        const { _id, userId: _, ...updateData } = resumeDataCopy;

        const resume = await Resume.findOneAndUpdate({ userId, _id: resumeId }, updateData, { new: true });
